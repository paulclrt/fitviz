...
async function fetchFitbitAPI(token: string, endpoint: string) {
...
// Heart rate data
router.get("/heart-rate/:date", async (req: Request, res: Response) => {
  try {
...
// ECG data
router.get("/ecg/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
...
// Calories
router.get("/calories/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
...
// Sleep data
router.get("/sleep/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
...
// Steps
router.get("/steps/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
    const data = await fetchFitbitAPI(token, `/1/user/-/activities/date/${date}.json`);
...
// Distance
router.get("/distance/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
    const data = await fetchFitbitAPI(token, `/1/user/-/activities/date/${date}.json`);
    const distance = data.summary.distances.find((d: any) => d.activity === "total").distance;
    res.json(distance);
  } catch (err) {
    res.status(400).json({ error: (err as Error).message });
  }
...
// Floors
router.get("/floors/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
    const data = await fetchFitbitAPI(token, `/1/user/-/activities/date/${date}.json`);
...
// Active minutes
router.get("/active-minutes/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
    const data = await fetchFitbitAPI(token, `/1/user/-/activities/date/${date}.json`);
...
// Sedentary minutes
router.get("/sedentary-minutes/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
    const data = await fetchFitbitAPI(token, `/1/user/-/activities/date/${date}.json`);
...
// Recent activity
router.get("/recent-activity/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
...
// HRV Daily
router.get("/hrv-daily/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
...
// HRV Continuous
router.get("/hrv-continuous/:date", async (req: Request, res: Response) => {
  try {
      let token: string = req.token;
    const { date } = req.params;
